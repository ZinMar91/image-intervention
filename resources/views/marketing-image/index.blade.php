    ...
    @if($marketingImages->count() > 0)
        <table class="table table-hover table-bordered table-striped">
            ...
            @foreach($marketingImages as $marketingImage)
                <tr>
                    <td><a href="/marketing-image/{{ $marketingImage->id }}/edit"> {{ $marketingImage->id }}</a></a>
                    </td>
                    <td><a href="/marketing-image/{{ $marketingImage->id }}"> <img
                                    src="{{ $marketingImage ->showImage($marketingImage, $thumbnailPath) }}"> </a></td>
